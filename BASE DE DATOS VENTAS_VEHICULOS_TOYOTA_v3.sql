...
Bienvenidos! En la actualidad, la gestión y análisis de datos se ha vuelto cada vez más importante para las empresas 
y organizaciones en todo el mundo. El modelado de datos es un proceso fundamental para lograr una gestión efectiva 
de la información, y se utiliza en una gran variedad de campos, desde la gestión de inventarios hasta la investigación médica.

En este artículo, nos enfocaremos en el modelado de datos para un sistema de ventas de vehículos de la marca Toyota
utilizando SQL Server como gestor de base de datos. A través de este ejemplo, aprenderemos cómo diseñar una base de 
datos relacional, cómo crear tablas y relaciones, cómo agregar índices y cómo extraer información para análisis y
visualización de datos.

Además, exploraremos la importancia del análisis de datos en el proceso de toma de decisiones empresariales, y 
cómo podemos utilizar las herramientas de análisis de datos para identificar tendencias, patrones y oportunidades.
En última instancia, veremos cómo el modelado y análisis de datos pueden contribuir al desarrollo de software y 
sistemas empresariales más efectivos y rentables.

Acompáñanos en este recorrido por el fascinante mundo del modelado y análisis de datos, y descubre cómo puedes 
utilizar esta herramienta para impulsar tu negocio hacia el éxito.
...
CREATE DATABASE VENTAS_VEHICULOS_TOYOTA_2023_JPV_0;
GO

--USAR LA BASE DE DATOS:
USE VENTAS_VEHICULOS_TOYOTA_2023_JPV_0;
GO
...
--ejemplo de cómo insertar 18 registros en cada tabla: 2020
...
/*
Un trigger, en el contexto de la programación y las bases de datos, es un tipo de objeto o componente que se utiliza para 
automatizar ciertas acciones o eventos en respuesta a cambios o condiciones específicas en la base de datos.

Un trigger se activa cuando ocurren ciertos eventos, como la inserción, actualización o eliminación de datos en una tabla de
la base de datos. Puede ser programado para ejecutar una serie de instrucciones o acciones definidas por el programador.

El objetivo principal de un trigger es garantizar la integridad de los datos y mantener la consistencia en la base de datos.
Puede utilizarse para realizar validaciones, aplicar reglas de negocio, realizar cálculos automáticos o realizar acciones
específicas en función de los eventos que desencadenen su ejecución.
...
SELECT MONTH(v.Fecha_Venta) as Mes, COUNT(vc.ID) as Numero_Ventas, vc.Modelo
FROM Ventas v
JOIN Vehiculos vc ON v.ID_Vehiculo = vc.ID
GROUP BY MONTH(v.Fecha_Venta), vc.Modelo;
...
CREATE VIEW VistaCalendario AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
    DATEPART(DAY, fecha_venta) AS DiaNumero,
    FORMAT(fecha_venta, 'dddd', 'es-ES') AS DiaLetras,
    DATEPART(WEEK, fecha_venta) AS SemanaNumero,
    FORMAT(fecha_venta, 'dddd', 'es-ES') AS SemanaLetras,
    CASE 
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 1 AND 3 THEN 'Primer Cuatrimestre'
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 4 AND 6 THEN 'Segundo Cuatrimestre'
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 7 AND 9 THEN 'Tercer Cuatrimestre'
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 10 AND 12 THEN 'Cuarto Cuatrimestre'
    END AS Cuatrimestre
FROM ventas;


SELECT * FROM VistaCalendario


CREATE or alter VIEW VistaCalendario AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
    ...
    FORMAT(fecha_venta, 'dddd', 'es-ES') AS SemanaLetras,
      CASE 
        WHEN DATEPART(QUARTER, fecha_venta) = 1 THEN 'Primer Trimestre'
        WHEN DATEPART(QUARTER, fecha_venta) = 2 THEN 'Segundo Trimestre'
        WHEN DATEPART(QUARTER, fecha_venta) = 3 THEN 'Tercer Trimestre'
        WHEN DATEPART(QUARTER, fecha_venta) = 4 THEN 'Cuarto Trimestre'
    END AS Trimestre,
	CASE 
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 1 AND 3 THEN 'Primer Cuatrimestre'
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 4 AND 6 THEN 'Segundo Cuatrimestre'
    ...
    CASE 
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 1 AND 6 THEN 'Primer Semestre'
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 7 AND 12 THEN 'Segundo Semestre'
    END AS Semestre
FROM ventas;


SELECT * FROM VistaCalendario2

CREATE OR ALTER VIEW VistaCalendario2 AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
    ...
    END AS Trimestre,
    CASE 
        WHEN DATEPART(MONTH, fecha_venta) <= 6 THEN 'Primer Semestre'
...
--DIaa de fiasta en la rep. dom

CREATE TABLE DiasDeFiesta (
    Fecha DATE,
    NombreFiesta VARCHAR(255)
);

--insertamos los dias de fiestas en rep. dom.
INSERT INTO DiasDeFiesta (Fecha, NombreFiesta)
VALUES
('2023-01-01', 'Año Nuevo'),
('2023-01-06', 'Día de Reyes'),
('2023-01-21', 'Día de la Altagracia'),
('2023-02-27', 'Día de la Independencia'),
('2023-04-06', 'Viernes Santo'),
('2023-05-01', 'Día del Trabajo'),
('2023-05-30', 'Día de Juan Pablo Duarte'),
('2023-06-20', 'Corpus Christi'),
...
CREATE OR ALTER VIEW VistaCalendario_Dias_De_Fiestas AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
    ...
    END AS Semestre,
    CASE 
        WHEN EXISTS (SELECT 1 FROM DiasDeFiesta WHERE Fecha = fecha_venta) THEN 'Sí' 
        ELSE 'No' 
    END AS EsDiaDeFiesta
FROM ventas;


SELECT * FROM VistaCalendario_Dias_De_Fiestas

SELECT COUNT(*) AS DiasDeFiesta
FROM VistaCalendario_Dias_De_Fiestas
WHERE EsDiaDeFiesta = 'No';

SELECT COUNT(*) AS DiasDeFiesta
FROM VistaCalendario_Dias_De_Fiestas
WHERE EsDiaDeFiesta = 'Sí';



CREATE OR ALTER VIEW VistaCalendario_Dias_De_Fiestas_TSC_Numero AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
    ...
    CASE 
        ...
        ELSE 'No' 
    END AS EsDiaDeFiesta,
    DATEPART(QUARTER, fecha_venta) AS TrimestreNumero,
    CASE 
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 1 AND 3 THEN 1
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 4 AND 6 THEN 2
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 7 AND 9 THEN 3
        WHEN DATEPART(MONTH, fecha_venta) BETWEEN 10 AND 12 THEN 4
    END AS CuatrimestreNumero,
    CASE 
        WHEN DATEPART(MONTH, fecha_venta) <= 6 THEN 1
        ELSE 2
    END AS SemestreNumero
FROM ventas;


SELECT * FROM VistaCalendario_Dias_De_Fiestas2


CREATE OR ALTER VIEW VistaCalendario_Dias_De_Fiestas2 AS
SELECT 
    fecha_venta AS Fecha,
    DATEPART(MONTH, fecha_venta) AS MesNumero,
    FORMAT(fecha_venta, 'MMMM', 'es-ES') AS MesLetras,
...
/*Aquí te presento algunos ejemplos de consultas avanzadas que se pueden realizar en la base de datos 
"Ventas_Vehiculos_Toyota_2023_V4" que hemos estado trabajando:

*/

--Subconsultas:Obtener el modelo y el precio de venta del vehículo con el precio de venta máximo en la tabla "Vehiculos":

SELECT Modelo, Precio_ventas 
FROM Vehiculos 
WHERE Precio_ventas = (SELECT MAX(Precio_ventas) FROM Vehiculos)


--JOIN:--Obtener el nombre del cliente, el modelo del vehículo y la fecha de venta de todas las ventas realizadas:

SELECT c.Nombre,ve.Modelo, v.Fecha_Venta
FROM Ventas v
INNER JOIN Clientes c ON v.ID_Cliente = c.ID
INNER JOIN Vehiculos ve ON v.ID_Vehiculo = ve.ID


--Funciones:--Obtener el precio de venta del vehículo con un descuento del 10%:

SELECT Modelo, Precio_ventas, Precio_ventas*0.10 AS Precio_Con_Descuento
FROM Vehiculos


--Triggers: --Crear un trigger que inserte un registro en la tabla "Ventas" cada vez que se inserte un registro en la tabla "Vehiculos":

CREATE TRIGGER Insertar_Venta_Auto 
ON Vehiculos 
AFTER INSERT 
AS 
BEGIN 
    INSERT INTO Ventas (ID_Vehiculo, ID_Cliente, ID_Vendedor, Fecha_Venta, Cantidad, Precio_Venta) 
    SELECT ID, 1, 1, GETDATE(), 1, Precio_ventas 
    FROM inserted 
END


--Vistas:	--Crear una vista que muestre el nombre y la dirección de los vendedores con más de 10 ventas realizadas:

CREATE VIEW Vendedores_Top_10 AS 
SELECT v.Nombre, v.Direccion
FROM vendedor v 
INNER JOIN (
    SELECT TOP 10 ID_Vendedor, COUNT(*) AS Total_Ventas 
    FROM Ventas 
    GROUP BY ID_Vendedor 
    HAVING COUNT(*) > 10
) vt ON v.ID = vt.ID_Vendedor

--selecionamos esa vista:

select * from Vendedores_Top_10



-- AHORA VAMOS A VER OTRAS CONSULTAS MÁS AVANZADAS DONDE VEREMOS EL PODER DEL ANÁLISIS DE DATOS 
-- Y CÓMO PODEMOS SACAR EL MÁXIMO PROVECHO A LAS CONSULTAS AVANZADAS QUE TIENE SQL SERVER:
...
--CREA UNA VISTA QUE MUESTRE ID, FECHA, CANTIDAD, TOTAl_PRECIO_COMPRA, TOTAL_PRECIO_VENTAS, MARGEN_BRUTO:
...
--CREA UNA VISTA QUE MUESTRE ID, FECHA, CANTIDAD, TOTAl_PRECIO_COMPRA, TOTAL_PRECIO_VENTAS, MARGEN_BRUTO Y MARGEN %:
...
/*
   Ahora podemos insertar las categorías correspondientes a cada modelo en la tabla Categoria y
   relacionarlos en la tabla Vehiculo_Categoria. Por ejemplo:
*/
...
/*
Vamos a crear una vista que muestre información de las tablas Categoria, Vehiculos, Ventas, 
Clientes y Vendedor, podemos unir las tablas mediante las claves foráneas y seleccionar los 
campos que deseamos mostrar. Aquí te dejo un ejemplo de cómo crear una vista:

*/

CREATE OR ALTER VIEW vista_ventas AS
SELECT v.ID AS ID_Vehiculo, v.Modelo, v.Anio, cat.Nombre AS Categoria, v.Precio_ventas, v.Stock,
       c.Nombre AS Nombre_Cliente, c.Telefono, c.Direccion, 
       vd.Nombre AS Nombre_Vendedor,
       ven.Fecha_Venta, ven.Cantidad, ven.Precio_Venta, ven.Total
FROM Vehiculos v
INNER JOIN Vehiculo_Categoria vc ON v.ID = vc.ID_Vehiculo
INNER JOIN Categoria cat ON vc.ID_Categoria = cat.ID
INNER JOIN Ventas ven ON v.ID = ven.ID_Vehiculo
INNER JOIN Clientes c ON ven.ID_Cliente = c.ID
INNER JOIN vendedor vd ON ven.ID_Vendedor = vd.ID;
...
Vamos a agregar la categoría a la vista "VentasToyotaPorFecha_v2" utilizando la tabla 
"Vehiculo_Categoria". Aquí te dejo el código actualizado para que puedas utilizarlo:

*/

CREATE OR ALTER VIEW VentasToyotaPorFecha_v_2023 AS
SELECT V.ID AS ID_Venta, V.Fecha_Venta, p.NOMBRE_PAIS as Pais,
...
CREATE OR ALTER VIEW VentasToyotaPorFecha_v_2023_Sin_Repetir AS
...
INNER JOIN vendedor E ON V.ID_Vendedor = E.ID
INNER JOIN PAIS P ON c.ID_pais=p.ID_PAIS
GROUP BY E.Nombre, VH.Modelo;


--SELECCIONAMOS LA VISTA:

select * from VentasToyotaPorFecha_v_2023_Sin_Repetir

 CREATE OR ALTER VIEW Todo_PorFecha_v_2023_Sin_Repetir AS
SELECT 
    C.Nombre AS Cliente,
    E.Nombre AS Vendedor,
    VH.Modelo, 
    ...
    ((SUM(V.Cantidad) * (VH.Precio_ventas - VH.Precio_compra)) / (SUM(V.Cantidad) * VH.Precio_ventas)) * 100 AS "%_Margen"
...
INNER JOIN Modelos_URL M ON VH.ID = M.ID_Vehiculo
INNER JOIN Clientes C ON V.ID_Cliente = C.ID
GROUP BY C.Nombre ,E.Nombre, VH.Modelo, CT.Nombre, VH.Precio_compra, VH.Precio_ventas, VH.Stock

 
--SELECCIONAMOS LA VISTA:

select * from Todo_PorFecha_v_2023_Sin_Repetir

--Veamos una consultas con parámetros Reales

select * from ventas where fecha_venta >= '2020-01-01' and fecha_venta <= '2020-12-31' order by ID asc
go
...
as
select * from Ventas where fecha_venta >= @fecha_Inicial and fecha_venta <= @fecha_Final
go
...
SELECT TOP 10
    ...
    V.Cantidad,
    V.Monto,
    (V.Monto - (V.Monto * M.Margen)) AS Margen
FROM
    Ventas V
    INNER JOIN Modelos M ON V.ID_Modelos = M.ID_Modelo
ORDER BY
    V.Monto DESC;
...
SELECT TOP 300
    ...
    V.Cantidad,
    V.Precio_Venta,
    ((V.Precio_Venta - Veh.Precio_compra) / Veh.Precio_compra) * 100 AS Margen
FROM
    Ventas V
...
ORDER BY
    ...
    SUM(V.Cantidad) AS Total_Cantidad,
    SUM(V.Precio_Venta) AS Total_Monto,
    ((SUM(V.Precio_Venta) - SUM(Veh.Precio_compra)) / SUM(V.Precio_Venta)) * 100 AS Margen
FROM
    Ventas V
    INNER JOIN Vehiculos Veh ON V.ID_Vehiculo = Veh.ID
GROUP BY
